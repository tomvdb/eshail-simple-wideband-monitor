...
var fft_colour = 'rgba(248,197,27,0.53)';
...
function updateFFT(data) {
    ...
    ctx.textAlign = "center";
    ctx.fillText("OSCAR 100 Offline Wideband Spectrum", canvasWidth / 2, 80);
    ctx.font = "15px ABeeZee";
    ctx.fillStyle = "white";
    ctx.textAlign = "center";
        ...
        for (i = 0; i < signals.length; i++) {
            if (mouse_x > signals[i].start
                ...
                if ((beacon_strength > 0) && (signals[i].start > canvasWidth / 8)) {
                    ctx.font = (signals[i].symbolrate < 500 ? "11px" : "18px") + " ABeeZee";
                    ...
                    x = ((beacon_strength_pixel - signals[i].top) / db_per_pixel).toFixed(1);
                    text_to_display = x.toString() + " dBb";
                    label_y_pos_offset = -20
                    if (x > 0) {
                        text_to_display = "+" + x.toString() + " dBb";
                    ...
                    //text_to_display = rx_status.mer / 10.0
                    /*
                    ctx.fillText( text_to_display,
                        signals[i].start - ((signals[i].start - signals[i].end)/2),
                        (canvasHeight * 7/8) - (7*((canvasHeight * 7/8) - signals[i].top)/8  +label_y_pos_offset+20));
                  */
                }
